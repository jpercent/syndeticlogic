...
def output(partId, ch_aux):
  ...
  if partId == 0:
    ...
    start = time()
    child = Popen(['./rank.sh', '1', test_file.name], stdout=PIPE, stderr=PIPE, shell=False);
    ...
    if (len(guesses) != linesOutput):
      ...
      start = time()
      child = Popen(['./rank.sh', '2', test_file.name], stdout=PIPE, stderr=PIPE, shell=False)
      (res, err) = child.communicate("")
      elapsed = time() - start
      guesses = res.splitlines()
      print err
      ...
      start = time()
      child = Popen(['./rank.sh', '3', test_file.name], stdout=PIPE, stderr=PIPE, shell=False)
      (res, err) = child.communicate("")
      elapsed = time() - start
      guesses = res.splitlines()
      print err
      ...
      start = time()
      child = Popen(['./rank.sh', '4', test_file.name], stdout=PIPE, stderr=PIPE, shell=False)
      (res, err) = child.communicate("")
      elapsed = time() - start
      guesses = res.splitlines()
      print err
