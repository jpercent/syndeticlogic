...
/**
 * This operator will feed all GameData objects to it's inner sub process and
 * will execute it in order to build an example set from the extracted
 * key value pairs.
 * 
...
public class ProcessGameDataOperator extends OperatorChain {
	...
	public ProcessGameDataOperator(OperatorDescription description) {
		...
		/** very short and insufficient meta data transformation: Should be much 
		 *  more sophisticated. 
		 */
	...
	public void doWork() throws OperatorException {
		...
		for (GameData gameData: loadedData) {
			...
			GameDataIOObject result = innerGameDataSink.getData();
	...
	/**
	 * This method has to extend the given resultSet by the example extracted from 
	 * the result object.
	 */
	...
	/**
	 * This will create the first initial example set from the result object.
	 * At first the MemoryExampleTable will be created to storing the data, then
	 * for each entry in the map an attribute is created and put together into an
	 * example set.
	 */
